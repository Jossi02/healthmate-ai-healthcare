...
export default function BottomNav() {
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 z-50 border-t border-gray-200/80 bg-white/90 shadow-[0_-4px_24px_-8px_rgba(0,0,0,0.08)] backdrop-blur-md transition-all duration-300">
      <div className="relative mx-auto flex h-20 max-w-md items-center justify-between px-6">
        {navItems.map((item) => {
          ...
          return (
            ...
            >
              <div className={`flex flex-col items-center justify-center space-y-1.5 transition-all duration-300 ${isActive ? '-translate-y-1 text-[#2563eb]' : 'text-gray-400 group-hover:text-gray-600 group-hover:-translate-y-0.5'}`}>
                {isActive && (
                  <span className="absolute -top-1 h-1 w-8 rounded-b-full bg-[#2563eb] shadow-[0_2px_8px_rgba(37,99,235,0.4)]" />
                )}
