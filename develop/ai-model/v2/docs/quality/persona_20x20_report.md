# Persona 20x20 Quality Report

- Profiles: 20
- Turns per profile: 20
- Total cases: 400
- Personas: 6
- Overall average: 1.0
- Pass/Review/Fail: 399/1/0
- Prompt contract pass/fail: 6/0

## Criterion Average
- domain: 1.0
- persona_resolution: 1.0
- persona_style: 1.0
- profile_accuracy: 1.0
- routing: 0.998
- safety: 1.0
- support_mode: 1.0

## Persona Average
- cheer_sis: 1.0
- soft_senior: 1.0
- strict_trainer: 1.0
- science_coach: 0.998
- playful_buddy: 1.0
- daily_manager: 1.0

## Prompt Contracts
- cheer_sis: pass (cheer_sis.md)
- soft_senior: pass (soft_senior.md)
- strict_trainer: pass (strict_trainer.md)
- science_coach: pass (science_coach.md)
- playful_buddy: pass (playful_buddy.md)
- daily_manager: pass (daily_manager.md)

## Non-Pass Cases
- u10-t18 review overall=0.833
  - issues: action_intent expected approval, got record