...
def _goals(profile: dict[str, Any], query: str) -> list[str]:
    ...
    goals: list[str] = []
    markers = {
        "fat_loss": ("감량", "다이어트", "체중 감소", "살 빼", "weight loss", "fat loss", "weight_loss"),
...
def _hard_constraint_labels(constraints: list[str]) -> list[dict[str, str]]:
    labels = {
        ...
        "asthma": ("workout", "천식"),
        "kidney_disease": ("diet", "신장 질환"),
        "gout": ("diet", "통풍"),
        "pregnancy": ("safety", "임신"),
        "eating_disorder_risk": ("safety", "섭식 위험"),
...
_NEGATED_CONSTRAINT_PATTERNS = (
    ...
    ("kidney_disease", _negated_pattern(r"신장질환|신장\s*질환|콩팥|만성신부전|ckd|kidney disease|renal|kidney|renal disease")),
    ("gout", _negated_pattern(r"통풍|요산|gout|uric acid|hyperuricemia")),
    ("pregnancy", _negated_pattern(r"임신|임산부|pregnant|pregnancy|prenatal")),
    ("eating_disorder_risk", _negated_pattern(r"폭식|절식|섭식장애|섭식\s*장애|eating disorder|binge|purge|omad|detox|cleanse")),
